...
function cookCoffee(name, buttonElement) {
 ...
 // let cupImg = document.querySelector(".coffee-cup img"); стала глобальной здесь будет видна 
...
function takeMoney(event) {
 ...
 //console.log(event.clientX, event.clientY);// clientX, отображает положение курсора по оси X, clientY соответственно по оси Y
...
changeButton.onclick = function (){
  ...
  if (coins.length == 0) {
    ...
    changeButton.innerHTML = "Забрать здачу";
    takeChange();
  } else {
...
/*changeButton.onclick = function() {takeChange();}*/
function takeChange() {
  if(balanceInput.value == 0) {
    return;
  }
...
function tossCoin(cost) {
  ...
  coin.onclick = function() {//проверить сколько монеток остаётся в контейнере
    coin.remove();
  }
  /*changeBox.innerHTML =`
  img src "img/10rub.png" style="width:30px; height: 30px;
  `!!! дописать!!!*/
}
